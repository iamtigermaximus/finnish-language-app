  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful assistant specialized in Finnish language education. Provide accurate Finnish verbal noun forms and translations. Return ONLY valid JSON with the exact structure specified."
        },
        {
          role: "user",
          content: prompt
...
function getFallbackVerbalNoun(verb: string): VerbalNounAnalysis {
  const fallbackVerbs: Record<string, VerbalNounAnalysis> = {
    'opiskella': {
      ...
      notes: "The -minen form is the most common verbal noun type in Finnish, indicating the action or process itself.",
      isFinnishInput: true
    },
    'lukea': {
      verb: "lukea",
    ...
    'tehdä': {
      ...
      notes: "Tekeminen can refer to both the process of doing/making and the things that are made.",
      isFinnishInput: true
    },
    'kirjoittaa': {
      verb: "kirjoittaa",
      ...
      notes: "Kirjoittaminen refers to the activity of writing, while 'kirjoitus' would be a specific piece of writing.",
      isFinnishInput: true
    }
  };
  ...
  return {
    ...
    notes: "This is a generated example. The actual verbal noun form may vary based on verb type and consonant gradation rules.",
